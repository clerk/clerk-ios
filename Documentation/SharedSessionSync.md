...
2. Same token or same Client ID: the devices share one server-side Client, so the newer snapshot wins (server date, then `Client.updatedAt`). Signing in or out rotates the device token but keeps the Client.
