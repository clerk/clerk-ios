...
This audit retires only these four files. Separate audits subsequently retire the [shared coordinator](shared-session-coordinator-test-audit.md), [watch protocol](watch-replication-test-audit.md) and [general Clerk suite](clerk-test-retirement.md), and migrate [live integration](integration-test-audit.md). The shared-adoption suite remains for its access-group checks. No test target is disabled, and physical signed-in upgrade and entitlement gates remain open.
