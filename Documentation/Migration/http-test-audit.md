...
The custom middleware-ordering file is now retired under the [response middleware audit](response-middleware-test-audit.md). The mixed transport/state networking file still contains unresolved identity assertions. These tests do not replace live TLS, device network interruption, upload size/performance or backend auth checks. URLSession's existing 16 MiB response validation occurs after loading; it is a decoded-response limit, not a proven peak-memory bound.
