...
| Failed/successful tokenless startup requests | Removed native startup refresh coordinator. Awaited connection changes the available call sequence: no primary auth owner is returned during startup. The complete request construction audit records that intentional change alongside current identity/race evidence. |
| Frozen shared identity on retry; identity/device-token changes cancel retry; custom signing sees frozen values | Removed custom native middleware and shared-session context. The embedded core owns retries/credential epochs. Live shared-session synchronization is unavailable. The request retry audit now records obsolete-transmission regressions, their fix and removed custom signing contracts. |
...
The custom middleware-ordering file is now retired under the [response middleware audit](response-middleware-test-audit.md). The [complete request construction audit](request-construction-test-audit.md) now retires the mixed API-client and header-middleware files after mapping every declaration and recording changed startup/metadata contracts. These tests do not replace live TLS, device network interruption, upload size/performance or backend auth checks. URLSession's existing 16 MiB response validation occurs after loading; it is a decoded-response limit, not a proven peak-memory bound.
