...
| `LocaleUtilsTests` | The two shape checks exercise the removed native `LocaleUtils.userLocale()` helper. They do not prove localized UI or request negotiation. Request locale continuity is now tested separately through the shared core and both packaged runtimes; see [Authentication locale continuity](locale.md). These helper shape checks alone do not establish that behavior. |
| `ExternalAuthUtilsTests` | The six nonce-parser cases are now owned by TypeScript callback orchestration. Existing SSO tests check nonce forwarding and reject an unrelated callback route. The subsequent [callback audit](callback-test-audit.md) verifies empty/missing nonce and fragment cases through the real core, including the intentionally omitted empty nonce request value. |
...
| `SessionUtilsTests` | The five selection and eleven comparator cases are mapped in the [session assertion audit](session-utility-test-audit.md). Twelve shared selection/reload tests and both packaged native proofs validate canonical selection, post-await state, stale old handles and readable returned resources. The old boolean comparator is removed. |
...
`SessionUtilsTests` is retired after its separate assertion audit and passing generated reload proofs. Other core identity/response-ordering suites remain retained. This retirement does not claim JWT, callback, session-transition or proxy
