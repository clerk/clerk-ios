...
Unreviewed old tests remain retained while assertion-level platform and coverage migration is incomplete. The browser/passkey assertion audits below record three reviewed file retirements after passing replacement tests. No file in this inventory is claimed to pass unchanged against the new API. Do not remove tests merely because the generated API compiles. Domain tests that only assert calls into a deleted native service should be retired with that service; retained public behavior must be checked through its current owner. The lists below identify where that decision belongs and explicitly preserve unresolved gates.
