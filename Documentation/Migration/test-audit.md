...
All 108 inventoried files have completed their assertion/helper review: 105 are retired and three integration files are migrated. The per-domain audits record replacement checks and explicitly removed contracts. No file in this inventory is claimed to pass unchanged against the new API. Tests were not retired merely because the generated API compiles: retained public behavior is checked through its current owner. The lists below preserve the source inventory and unresolved release gates. The [final adoption retirement](shared-adoption-test-audit.md#final-retirement-decision) removes the empty legacy target and wires `make test` to the complete native core suite.
...
recursive 401 client-recovery loop in TypeScript. The [credential clear audit](client-credential-clear-test-audit.md) retires three device-token middleware declarations after fixing explicit server credential deletion and Expo custom-cache clearing. The partial [request retry audit](request-retry-test-audit.md) maps four API-client declarations and fixes obsolete transmissions plus Android fractional timers. The [complete request construction audit](request-construction-test-audit.md) retires all nineteen API-client and thirteen header-middleware declarations. The [client-sync middleware audit](client-sync-middleware-test-audit.md) retires its nineteen declarations after completing the source/state/presentation map. These networking suites are now retired; the general core, shared-session and storage suites still require their own audits.
...
The [unused test-support audit](unused-test-support-audit.md) retires the generic native-service helpers and JWT fixture helper after the Clerk suite retirement leaves them with no Swift callers. The [final adoption audit](shared-adoption-test-audit.md#final-retirement-decision) subsequently retires the shared-adoption suite and its in-memory Keychain helper after the source-selection correction and inactive App Attest call-site review.
