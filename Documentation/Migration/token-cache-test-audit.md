...
All assertion bodies were reviewed in baseline `02f98f89a19b6c079517c9aae07df7edd0e600e5` for `Tests/Domains/Auth/TokenFreshnessTests.swift` and `Tests/Domains/Auth/Session/SessionTokenFetcherTests.swift`. The former has 14 declarations. The latter has 24 declarations, including a multiline parameterized declaration missed by the earlier file-level inventory's count of 23. Both files are retired after the snapshot follow-up below closes their remaining deterministic execution checks. Live service and actual old-major upgrades remain separate release gates.
...
| `retainedSessionDoesNotRehydrateInvalidatedSnapshot`, `nonActiveSessionDoesNotReturnExistingCachedToken`, `concurrentNonActiveSessionFetchesShareRequest` | The old tests continue fetching through a retained session after removing it from the client. The generated root handle instead becomes stale when removed, as checked by `session-state.test.mjs` and native packaged checks. The session service follow-up checks concurrent requests and cache hits for a nonselected session still available in `Clerk.sessions`. |
| `transitionedActiveSessionCanHydrateCanonicalSnapshot` | The `initial` snapshot case verifies a zero-request read from the canonical startup snapshot. The generated `clearCache()` intentionally differs from the removed private invalidation helper: the `clear` case verifies that a subsequent read makes a new mint request and uses the existing last-active token as mint input. |
...
| `hydrationDoesNotReplaceCanonicalTokenOnTimestampTie`, `hydrationAcceptsStrictlyFresherSessionSnapshot`, `hydrationPreservesFresherExpiredTokenForNextMint` | The `mintTie` case verifies that an equal-timestamp forced mint remains cached on the next ordinary read. Reload cases verify that stale snapshots do not displace a fresher mint, fresh snapshots populate the cache, and a fresher expired token survives a stale expired snapshot as the next mint's input. A newly received snapshot on a full timestamp tie wins under canonical TypeScript policy (`tie`); the old private hydration-only tie preference is removed. |
...
At this initial stage, snapshot-hydration execution was still unresolved; the follow-up below closes that deterministic gap. These fixtures do not establish live service behavior or signed-in old-major upgrades. Native token/session policy remains in TypeScript throughout.
...
The [session service audit](session-service-test-audit.md) adds generated-call evidence for mint request bodies, a retained session's template token after an accepted organization switch, and concurrent requests/cache hits for a nonselected session still available in `Clerk.sessions`. These close the corresponding execution gaps above. Scoped sign-out clears the shared cache globally before HTTP, including on failure; all-session removal clears after success. The old private scoped deletion contract is not restored. Live upgrades remain unverified.

## Snapshot execution and retirement

`packages/mobile-runtime/test/token-snapshot.test.mjs`, native Swift `TokenSnapshotTests.swift`, and Android `TokenSnapshotTest.kt` execute seven matching scenarios through generated `Session.getToken`, `clearCache`, and `reload` calls. They verify actual token strings, request counts, previous-token mint bodies, and retained session handles. No private native cache is recreated or inspected.

The scenarios cover initial hydration without minting; a new request after cache clearing; an equal-timestamp mint surviving a later cache read; stale, fresh, and equal-timestamp client snapshots; and an expired token retaining the fresher mint input after a stale expired snapshot. The changed tie, invalidation, removal, and waiter-cancellation contracts are explicit in the assertion map above rather than claimed as old API compatibility.

All seven embedded scenarios and seven Android instrumentation cases pass. The iOS Simulator packaged-core suite passes 29 test declarations, including all seven snapshot parameter cases; the macOS suite passed with the first six cases before the final equal-timestamp mint case was added. The 200 source tests across `Session`, `tokenCache`, and `tokenFreshness` pass. No production code or packaged bundle changes were required: the core remains at `ea3c1dd6d3ba43f411f2152566d35640a1038104`, SHA-256 `48237898bd4e60de4ffb99c0a5d0c2a8bd06be8c322e2b5964ecbfab316a9d76`.

These checks and the complete assertion map justify retiring exactly the two obsolete Swift token suites. They do not retire the separate identity, middleware, integration, or shared-session tests, and do not establish live HTTP acceptance, physical suspension, or signed-in upgrades.
