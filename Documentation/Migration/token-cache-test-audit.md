...
This audit does not establish automatic background-refresh invalidation, cancellation behavior for multiple callers sharing one token request, all old snapshot-hydration semantics, live service behavior, or signed-in old-major upgrades. The legacy files remain available for those follow-up checks. Native token/session policy remains in TypeScript throughout.
