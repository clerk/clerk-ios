...
@MainActor enum CredentialUpgradeProof {
  static func run(mode: String, fixture: String) async throws {
    ...
    case "clear": try await storage.remove()
    case "assert-cleared":
      let token = try await storage.read()
      precondition(token == nil)
    case "cleanup":
      for (service, account) in [(identityService, "clerkSharedSessionLocalIdentityV2"), (application, "clerkDeviceToken"), (application + ".clerk.core.v2." + hash(key), "client"), (application + ".clerk.core.v2." + hash(key + "-other"), "client")] {
