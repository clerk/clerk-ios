...
@Suite(.serialized) struct SignInSerializedTests {
  ...
  @Test(
    "All prepare second factor strategies for client trust",
    arguments: [
      SignIn.PrepareSecondFactorStrategy.phoneCode,
      SignIn.PrepareSecondFactorStrategy.emailCode()
    ])
  @MainActor
  func testPrepareSecondFactorForClientTrust(strategy: SignIn.PrepareSecondFactorStrategy) async throws {
    let requestHandled = LockIsolated(false)
    let mockSignIn = SignIn.mock
    let signIn = SignIn(
      id: mockSignIn.id,
      status: .needsClientTrust,
      supportedIdentifiers: mockSignIn.supportedIdentifiers,
      identifier: mockSignIn.identifier,
      supportedFirstFactors: mockSignIn.supportedFirstFactors,
      supportedSecondFactors: [
        .mockEmailCode,
        .mockPhoneCode
      ],
      firstFactorVerification: mockSignIn.firstFactorVerification,
      secondFactorVerification: mockSignIn.secondFactorVerification,
      userData: mockSignIn.userData,
      createdSessionId: mockSignIn.createdSessionId
    )
    let params = strategy.params(signIn: signIn)
    let originalUrl = mockBaseUrl.appending(path: "/v1/client/sign_ins/\(signIn.id)/prepare_second_factor")
    var mock = Mock(
  ...
  @Test(
    "All attempt second factor strategies for client trust",
    arguments: [
      SignIn.AttemptSecondFactorStrategy.phoneCode(code: "phonecode"),
      .emailCode(code: "emailcode")
    ])
  @MainActor
  func testAttemptSecondFactorForClientTrust(strategy: SignIn.AttemptSecondFactorStrategy) async throws {
    let requestHandled = LockIsolated(false)
    let mockSignIn = SignIn.mock
    let signIn = SignIn(
      id: mockSignIn.id,
