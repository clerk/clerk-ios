...
struct ClerkAPIClientTests {
  ...
  @Test
  func blockedIdentityOnlyAllowsOptedOutRequests() async throws {
    let clerk = Clerk()
    ...
    coordinator.block(ownership, reason: .storageUnavailable)

    let environmentURL = URL(
      string: mockBaseUrl.absoluteString + "/v1/environment"
    )!
    var environmentPayload = try #require(
      JSONSerialization.jsonObject(
        with: JSONEncoder.clerkEncoder.encode(Clerk.Environment.mock)
      ) as? [String: Any]
    )
    var responseClient = Client.mock
    responseClient.id = "response-client"
    environmentPayload["client"] = try #require(
      JSONSerialization.jsonObject(
        with: JSONEncoder.clerkEncoder.encode(responseClient)
      ) as? [String: Any]
    )
    let identityHeadersWereOmitted = LockIsolated(false)
    var mock = try Mock(
      url: environmentURL,
      ignoreQuery: true,
      contentType: .json,
      statusCode: 200,
      data: [
        .get: JSONSerialization.data(withJSONObject: environmentPayload),
      ],
      additionalHeaders: ["Authorization": "response-device-token"]
    )
    mock.onRequestHandler = OnRequestHandler { @Sendable request in
      identityHeadersWereOmitted.setValue(
        request.value(forHTTPHeaderField: "Authorization") == nil
          && request.value(forHTTPHeaderField: "x-clerk-client-id") == nil
      )
    }
    mock.register()
    ...
    #expect(response.value == Clerk.Environment.mock)
    #expect(identityHeadersWereOmitted.value)
    #expect(clerk.client?.id == existingClient.id)

    await #expect(throws: ClerkClientError.self) {
      _ = try await apiClient.send(
        Request<EmptyResponse>(path: "/v1/authenticated")
      )
    }
  }
