...
struct ClerkDeviceTokenResponseMiddlewareTests {
  ...
  @Test
  func validateSkipsDeviceTokenHeaderWhileClearIsPending() async throws {
    configureClerkForTesting()
    let keychain = InMemoryKeychain()
  ...
  @Test
  func validateStoresDeviceTokenHeaderWhenStaleClearPendingHasStoredToken() async throws {
    configureClerkForTesting()
    let keychain = InMemoryKeychain()
    try keychain.set("current-token", forKey: ClerkKeychainKey.clerkDeviceToken.rawValue)
    try keychain.set("true", forKey: ClerkKeychainKey.clerkDeviceTokenClearPending.rawValue)
