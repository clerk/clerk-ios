...
public struct Session: Codable, Equatable, Sendable, Identifiable {
  ...
  public init(from decoder: Decoder) throws {
    ...
    factorVerificationAge = try container.decodeIfPresent([Int].self, forKey: .factorVerificationAge)
    expireAt = try container.decodeMillisecondsDate(forKey: .expireAt)
    abandonAt = try container.decodeMillisecondsDate(forKey: .abandonAt)
    lastActiveAt = try container.decodeMillisecondsDate(forKey: .lastActiveAt)
    lastActiveToken = try container.decode(Token.self, forKey: .lastActiveToken)
    lastActiveOrganizationId = try container.decodeIfPresent(String.self, forKey: .lastActiveOrganizationId)
    actor = try container.decodeIfPresent(ActClaim.self, forKey: .actor)
    tasks = try container.decodeIfPresent([SessionTask].self, forKey: .tasks)
    user = try container.decode(User.self, forKey: .user)
    publicUserData = try container.decode(PublicUserData.self, forKey: .publicUserData)
