...
package struct WatchSyncPayload {
  ...
  init?(applicationContext: [String: Any]) {
    ...
    guard hasDeviceTokenState == hasDeviceTokenVersion,
          hasAuthState == hasAuthVersion
    else {
      return nil
    }
    if hasDeviceTokenState {
      guard let state = applicationContext[Self.deviceTokenStateKey] as? String,
            ...
            let version = Self.decodeVersion(applicationContext[Self.deviceTokenVersionKey]),
            version >= .initial
      else {
        return nil
      }
    ...
    if hasAuthState {
      guard let state = applicationContext[Self.authStateKey] as? String,
            state == "set" || state == "cleared",
            let version = Self.decodeVersion(applicationContext[Self.authVersionKey]),
            version >= .initial
      else {
        return nil
      }
