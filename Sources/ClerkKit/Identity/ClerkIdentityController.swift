...
final class ClerkIdentityController {
  ...
  struct ExternalTransition {
    ...
    /// Completes a successfully staged transition after persistence returns.
    /// A thrown persistence error intentionally skips completion so the staged
    /// intent remains available for an exact retry.
    @MainActor
    func complete(didApply: Bool) {
      if didApply {
        self.didApply()
      } else {
        didNotApply()
...
extension ClerkIdentityController {
  ...
  func invalidateAndDrainLocalOperations(
    ...
    await localIdentityIO?.invalidateOperations(through: revision)
    await localOperationTail?.value
...
extension ClerkIdentityController {
  ...
  func submitExternalTransition(
    ...
    case .shared(let coordinator):
      return coordinator.enqueueSerializedLocalIdentityOperation { [weak self, weak clerk] in
        ...
        transition.complete(didApply: didApply)
      }
    case .atomicLocal(let localIdentityIO):
    ...
    case .legacy:
      ...
      transition.complete(didApply: true)
      return nil
    }
  }
  ...
  func updateDeviceToken(to deviceToken: String) async throws -> DeviceTokenTransitionResult {
    ...
    switch persistenceMode(for: clerk) {
    case .shared(let coordinator):
      let task: Task<DeviceTokenTransitionResult, Error> = coordinator
        .enqueueSerializedLocalIdentityOperation { [weak self, weak clerk] in
          guard let self,
                let clerk,
                case .shared(let currentCoordinator) = persistenceMode(for: clerk),
                currentCoordinator === coordinator
          ...
          guard coordinator.currentDeviceToken != deviceToken else { return .unchanged }
      ...
      return try await task.value
    case .atomicLocal(let localIdentityIO):
      let task: Task<DeviceTokenTransitionResult, Error> = enqueueLocalOperation { [weak self] operationRevision in
        guard let self else { throw CancellationError() }
        guard localDeviceToken != deviceToken else { return .unchanged }
        let identity = try clearedIdentity(with: deviceToken)
        let didApply = try await persistAndApplyAtomicIdentity(
          identity,
          through: localIdentityIO,
