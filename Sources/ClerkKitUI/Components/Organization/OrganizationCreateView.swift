...
struct OrganizationCreateView: View {
  ...
  let onCreated: (Organization) async throws -> Void
  ...
  init(
    ...
    onCreated: @escaping (Organization) async throws -> Void
  ) {
  ...
  private func createOrganization() async {
    ...
    do {
      ...
      try await onCreated(organization)
    } catch {
      self.error = error
    }
  }
...
#Preview("Create Organization") {
  NavigationStack {
    OrganizationCreateView(creationDefaults: nil) { _ in }
      .clerkPreview()
  }
}
