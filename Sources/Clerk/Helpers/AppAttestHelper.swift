...
struct AppAttestHelper {
  ...
  static func performDeviceAttestation() async throws -> String {
    ...
    try Clerk.shared.dependencyContainer.keychain.set(keyId, forKey: keychainKey)
    return keyId
  }
  ...
  static var hasKeyId: Bool {
    do {
      return try Clerk.shared.dependencyContainer.keychain.hasItem(forKey: keychainKey)
    } catch {
      return false
    }
  }
  ...
  private static var keyId: String? {
    try? Clerk.shared.dependencyContainer.keychain.string(forKey: keychainKey)
  }
  ...
  static func removeKeyId() throws {
    try Clerk.shared.dependencyContainer.keychain.deleteItem(forKey: keychainKey)
  }
  ...
  static var clientId: String? {
    do {
      guard let data = try Clerk.shared.dependencyContainer.keychain.data(forKey: "cachedClient") else {
        return nil
      }
