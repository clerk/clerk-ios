...
struct BackupCodesView: View {
  ...
  enum MfaType {
    ...
    var instructions: LocalizedStringKey {
      ...
      case .phoneCode:
        """
        When signing in, you will need to enter a verification code
        sent to this phone number as an additional step.

        Save these backup codes and store them somewhere safe.
        If you lose access to your authentication device, you can use
        backup codes to sign in.
        """
      case .authenticatorApp:
        """
        Two-step verification is now enabled. When signing in, you will need
        to enter a verification code from this authenticator app as an
        additional step.

        Save these backup codes and store them somewhere safe. If you lose
        access to your authentication device, you can use backup codes to sign in.
        """
      case .backupCodes:
        """
        Backup codes are now enabled. You can use one of these to sign in to
        your account if you lose access to your authentication device. Each
