...
extension Color {
  ...
  func mix(with color: Color, amount: CGFloat) -> Color {
    ...
    let mixedRed = from.red + (target.red - from.red) * amount
    let mixedGreen = from.green + (target.green - from.green) * amount
    let mixedBlue = from.blue + (target.blue - from.blue) * amount

    return Color(red: Double(mixedRed), green: Double(mixedGreen), blue: Double(mixedBlue))
  }
