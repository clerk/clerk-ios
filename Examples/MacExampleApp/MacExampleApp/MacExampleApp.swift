...
struct MacExampleApp: App {
  @State private var connection = MacExampleConnection()
...
private final class MacExampleConnection {
  ...
  func connect() async {
    ...
    do {
      let configuration = try ClerkConfiguration(
        publishableKey: MacExampleLocalSecrets.load().publishableKey ?? "",
        callbackURL: URL(string: "com.clerk.MacExampleApp://oauth/callback")!
      )
