...
struct CustomFlowsApp: App {
  @State private var connection = CustomFlowsConnection()
  @State private var feedback = CustomFlowFeedback()
...
private final class CustomFlowsConnection {
  ...
  func connect() async {
    ...
    do {
      let configuration = try ClerkConfiguration(
        publishableKey: CustomFlowsLocalSecrets.load().publishableKey ?? "",
        callbackURL: URL(string: "com.clerk.CustomFlows://oauth/callback")!
      )
