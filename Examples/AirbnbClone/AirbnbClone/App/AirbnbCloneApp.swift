...
struct AirbnbCloneApp: App {
  @State private var feedback = AirbnbAuthFeedback()
  @State private var connection = AirbnbCloneConnection()
...
private final class AirbnbCloneConnection {
  ...
  func connect() async {
    ...
    do {
      let configuration = try ClerkConfiguration(
        publishableKey: AirbnbCloneLocalSecrets.load().publishableKey ?? "",
        callbackURL: URL(string: "com.clerk.AirbnbClone://oauth/callback")!
      )
...
@MainActor @Observable
final class AirbnbAuthFeedback {
