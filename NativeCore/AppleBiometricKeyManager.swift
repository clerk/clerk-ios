//
//  AppleBiometricKeyManager.swift
//  Clerk
//
...
enum AppleBiometricKeyPolicy: String {
...
extension AppleBiometricKeyManagerProtocol {
  @MainActor
  ...
  @MainActor
  func createKey() throws -> AppleBiometricLocalKey {
    try createKey(policy: .biometryCurrentSet)
  }

  @MainActor
  func sign(clientData: String, localKeyId: String) throws -> AppleBiometricSignature {
    try sign(clientData: clientData, localKeyId: localKeyId, localizedReason: nil)
  }
}

final class AppleBiometricKeyManager: AppleBiometricKeyManagerProtocol {
  private static let applicationTagPrefix = "dev.clerk.trusted_device"

  @MainActor
  func isSupported(policy: AppleBiometricKeyPolicy) -> Bool {
  ...
  @MainActor
  func createKey(policy: AppleBiometricKeyPolicy = .biometryCurrentSet) throws -> AppleBiometricLocalKey {
    #if (os(iOS) || os(macOS)) && canImport(LocalAuthentication)
    guard canCreateKey(policy: policy) else {
      throw AppleBiometricKeyError.biometricAuthenticationUnavailable
    }
    ...
    guard let privateKey = SecKeyCreateRandomKey(attributes as CFDictionary, &error) else {
      throw AppleBiometricKeyError.keyGenerationFailed(Self.errorMessage(from: error))
    }
    ...
    #else
    throw AppleBiometricKeyError.unsupportedPlatform
    #endif
  }

  @MainActor
  private func canCreateKey(policy: AppleBiometricKeyPolicy) -> Bool {
    #if (os(iOS) || os(macOS)) && canImport(LocalAuthentication)
    let context = LAContext()
    var error: NSError?
  ...
  func sign(
    ...
    guard let signature = SecKeyCreateSignature(
      ...
      throw AppleBiometricKeyError.signingFailed(Self.errorMessage(from: error))
    }

    let rawSignature = try Self.rawES256Signature(fromDEREncoded: signature)

    return AppleBiometricSignature(
  ...
  func hasKey(localKeyId: String) throws -> Bool {
    ...
    case errSecItemNotFound:
      return false
    default:
      throw CoreError(code: "secure_storage_error_\(status)")
    }
  }
  ...
  func deleteKey(localKeyId: String) throws {
    ...
    case errSecSuccess, errSecItemNotFound:
      return
    default:
      throw AppleBiometricKeyError.deletionFailed(status)
    }
  }
  ...
  private func privateKey(localKeyId: String, localizedReason: String?) throws -> SecKey {
    ...
    case errSecSuccess:
      guard let privateKey else {
        throw AppleBiometricKeyError.keyNotFound
      }
      return privateKey
    case errSecItemNotFound:
      throw AppleBiometricKeyError.keyNotFound
  ...
  static func accessControlFlags(for policy: AppleBiometricKeyPolicy) -> SecAccessControlCreateFlags {
    switch policy {
  ...
  static func makeAccessControl(policy: AppleBiometricKeyPolicy = .biometryCurrentSet) throws -> SecAccessControl {
    var error: Unmanaged<CFError>?
    guard let accessControl = SecAccessControlCreateWithFlags(
      ...
      throw AppleBiometricKeyError.keyGenerationFailed(errorMessage(from: error))
    }

    return accessControl
  }

  static func makePrivateKeyAttributes(
    localKeyId: String,
  ...
  static func privateKeyLookupError(for status: OSStatus) -> Error {
    switch status {
    case errSecUserCanceled:
      AppleBiometricKeyError.biometricAuthenticationCanceled
    case errSecAuthFailed:
      AppleBiometricKeyError.biometricAuthenticationFailed
    case errSecInteractionNotAllowed:
      AppleBiometricKeyError.biometricAuthenticationUnavailable
    default:
      CoreError(code: "secure_storage_error_\(status)")
    }
  }
  ...
  static func publicKeyJWK(fromX963Representation representation: Data) throws -> String {
    guard representation.count == 65, representation.first == 0x04 else {
      throw AppleBiometricKeyError.invalidPublicKey
    }
  ...
  @MainActor
  static func completeKeyCreation(
    localKeyId: String,
    policy: AppleBiometricKeyPolicy,
    exportPublicKeyJWK: () throws -> String,
    deleteKey: (String) throws -> Void
  ) throws -> AppleBiometricLocalKey {
    do {
      return try AppleBiometricLocalKey(
        localKeyId: localKeyId,
      ...
      do {
        ...
        ClerkLogger.error(
          "Failed to delete biometric-credential key after public-key export failed.",
          error: error
        )
      }
      throw exportError
  ...
  static func base64URLEncodedString(_ data: some DataProtocol) -> String {
    Data(data)
      .base64EncodedString()
  ...
  private static func publicKeyJWK(for privateKey: SecKey) throws -> String {
    guard let publicKey = SecKeyCopyPublicKey(privateKey) else {
      throw AppleBiometricKeyError.publicKeyExportFailed("Unable to copy public key.")
    }

    var error: Unmanaged<CFError>?
    guard let representation = SecKeyCopyExternalRepresentation(publicKey, &error) as Data? else {
      throw AppleBiometricKeyError.publicKeyExportFailed(errorMessage(from: error))
    }
  ...
  #if (os(iOS) || os(macOS)) && canImport(LocalAuthentication)
  static func localAuthenticationPolicy(for policy: AppleBiometricKeyPolicy) -> LAPolicy {
    switch policy {
  ...
  static func localAuthenticationPolicyForKeyCreation(for policy: AppleBiometricKeyPolicy) -> LAPolicy {
    switch policy {
  ...
  private static func invalidES256SignatureError() -> AppleBiometricKeyError {
...
struct AppleBiometricLocalKey: Equatable {
  let localKeyId: String
  let publicKeyJWK: String
  let algorithm: String
  let policy: AppleBiometricKeyPolicy

  init(
    localKeyId: String,
    publicKeyJWK: String,
    algorithm: String = "ES256",
    policy: AppleBiometricKeyPolicy = .biometryCurrentSet
  ) {
...
struct AppleBiometricSignature: Equatable {
...
enum AppleBiometricKeyError: Error, Equatable, LocalizedError {
...
protocol AppleBiometricKeyManagerProtocol: Sendable {
  @MainActor func isSupported(policy: AppleBiometricKeyPolicy) -> Bool
  @MainActor func createKey(policy: AppleBiometricKeyPolicy) throws -> AppleBiometricLocalKey
  @MainActor func sign(
    clientData: String,
    localKeyId: String,
