...
def summarize(report):
    ...
    for phase in ['before', 'startup', 'warmup', 'steadyWait', 'steady', 'close', 'closedWait', 'closed']:
    ...
    if phases['steady']['spanMilliseconds'] < 900 or phases['closed']['spanMilliseconds'] < 900:
...
def main():
    ...
    for index in range(args.pairs):
        ...
        for variant in order:
            ...
            console = run(['xcrun', 'devicectl', 'device', 'process', 'launch', '--device', args.device,
                           '--console', '--terminate-existing', '--timeout', '55',
                           '--json-output', str(stem.with_suffix('.launch.json')), bundle_id,
                           '--memory', '--exit-after-ready'], stem)
            if 'CLERK_CORE_MEMORY_WRITTEN ' not in console or 'CLERK_CORE_FOOTPRINT_READY ' + bundle_id not in console:
            ...
            run(['xcrun', 'devicectl', 'device', 'copy', 'from', '--device', args.device,
                 ...
                 '--source', 'Documents/core-memory.json', '--destination', str(destination),
            ...
            if variant == 'embedded' and (report.get('ownerCount') != 1 or not report.get('authenticatedAtReady')
                                         or not report.get('ownerReleasedAfterClose') or report.get('warmupResets') != 50):
        ...
        pair['incrementalBytes'] = {
            ...
            for phase, stat in [('startup', 'maximum'), ('steady', 'median'), ('closed', 'median')]
        }
        pairs.append(pair)
    ...
    result = {'schemaVersion': 1, 'model': args.model, 'nativeRevision': args.native_revision,
              ...
              'appIdentities': identities, 'pairCount': args.pairs, 'pairs': pairs,
              'limitations': ['One physical device; fixture HTTP and in-memory credentials',
                              ...
                              'Single-owner close observations are not a repeated connect/close stress test',
